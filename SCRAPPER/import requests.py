...
def scrapper_(id_):
	...
	try:
		with open ("F:/BACKUP/TP Final coder"+ str(id_) + ".txt", "w") as sp_txt:
			sp_txt.write(steam_price.text)

		with open ("F:/BACKUP/TP Final coder/scharts/sc"+ str(id_) + ".txt", "w") as sc_txt:
			sc_txt.write(steam_charts.text)
	except:
		with open ("F:/BACKUP/TP Final coder/ERROR/sp"+ str(id_) +"---ERROR---.txt", "w") as ERROR:
			ERROR.write("ERROR")
